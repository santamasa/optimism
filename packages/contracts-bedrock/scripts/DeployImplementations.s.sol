...
contract DeployImplementationsOutput is BaseDeployIO {
    ...
    ISystemConfigV160 internal _systemConfigImpl;
    ...
    function set(bytes4 _sel, address _addr) public {
        ...
        else if (_sel == this.systemConfigImpl.selector) _systemConfigImpl = ISystemConfigV160(_addr);
    ...
    function systemConfigImpl() public view returns (ISystemConfigV160) {
    ...
    function assertValidSystemConfigImpl(DeployImplementationsInput) internal view {
        ISystemConfigV160 systemConfig = systemConfigImpl();
...
contract DeployImplementations is Script {
    ...
    function deploySystemConfigImpl(DeployImplementationsInput _dii, DeployImplementationsOutput _dio) public virtual {
        ...
        ISystemConfigV160 impl;

        address existingImplementation = getReleaseAddress(release, contractName, stdVerToml);
        if (existingImplementation != address(0)) {
            impl = ISystemConfigV160(existingImplementation);
        } else {
            // Deploy a new implementation for development builds.
            vm.broadcast(msg.sender);
            impl = ISystemConfigV160(
                DeployUtils.create1({
                    _name: "SystemConfigV160.sol:SystemConfig",
                    _args: DeployUtils.encodeConstructor(abi.encodeCall(ISystemConfigV160.__constructor__, ()))
                })
            );
        }
        ...
        virtual
    {
        ...
        if (existingImplementation != address(0)) {
            ...
            impl = IL1CrossDomainMessenger(
                DeployUtils.create1({
                    _name: "L1CrossDomainMessengerV160.sol:L1CrossDomainMessenger",
                    _args: DeployUtils.encodeConstructor(abi.encodeCall(IL1CrossDomainMessengerV160.__constructor__, ()))
                })
            );
        }
        ...
        virtual
    {
        ...
        if (existingImplementation != address(0)) {
            ...
            impl = IL1StandardBridge(
                DeployUtils.create1({
                    _name: "L1StandardBridgeV160.sol:L1StandardBridge",
                    _args: DeployUtils.encodeConstructor(abi.encodeCall(IL1StandardBridgeV160.__constructor__, ()))
                })
            );
        }
